...
class RadarScenesBEVSegDataset(Dataset):
   ...
   def __init__(
       ...
       for path in npz_paths:
           ...
           start_time_path = time.time() # Start timer for individual path
           try:
               data = np.load(path, mmap_mode="r")
               T = data["bevs"].shape[0]
               data.close() # Close mmap object after reading metadata
           except Exception as e:
               print(f"[ERROR] Could not load {path}: {e}")
               continue
           end_time_path = time.time() # End timer for individual path
           # print(f"Time to process {path}: {end_time_path - start_time_path:.4f} seconds") # Optional: print individual times
       ...
       print(f"[init] Total time to process {len(npz_paths)} NPZ file metadata: {end_time_total - start_time_total:.4f} seconds")
   ...
   def __getitem__(self, idx: int):
       ...
       try:
           data = np.load(path, mmap_mode="r") # Use mmap_mode="r" for efficient loading
           ...
           if data is not None: # Ensure data was loaded before attempting to close
               data.close() # Close mmap object to release resources
